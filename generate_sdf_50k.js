// backend/scripts/generate_sdf_50k.js
const fs = require('fs');
const path = require('path');

// ============================================================
// TEMPLATES SDF (fusion des deux scripts)
// ============================================================
...
function generateDataset(total = 50000) {
    ...
    for (let i = 0; i < total; i++) {
        ...
        examples.push(ex);
        if ((i + 1) % 5000 === 0) console.log(`   ${i+1} exemples générés`);
    }
    return examples;
}

function saveDataset(examples, outputPath) {
    ...
    console.log(`   📊 ${examples.length} exemples`);
}
...
if (require.main === module) {
    ...
    const outputPath = path.join(projectRoot, 'datasets', 'sdf_synthetic_50k.jsonl');
    const examples = generateDataset(50000);
    saveDataset(examples, outputPath);
    console.log('\n🔍 Aperçu des 3 premiers exemples :');
    examples.slice(0, 3).forEach((ex, i) => console.log(`${i+1}. ${ex.text}`));
}
